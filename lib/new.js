...
// copy testing files
function copyTesting (obj) {
    copyTemplated(".travis.yml", obj);
    copyTemplated("package.json", obj);
    console.log("Testing support integrated, just flick the switch to enable Travis checking.");
    allOk();
}
    ...
    ,   function (err, res) {
            ...
            if (res.question) {
                ...
                if (fs.existsSync(jn(obj.dir, "index.html"))) {
                    ...
                    ,   function (err, res) {
                            if (err) die("Aborting due to error: " + err);
                            if (!res.question) return copyTesting(obj);
                            buildHTML(obj);
                        }
                    );
                }
